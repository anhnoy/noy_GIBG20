      ...
      <div>
        <table class="custom-table">
          ...
          <tr v-for="item in items" :key="item.id">
            ...
            <td >
              <p class="Business_status">{{ item.Business_status }}</p>
            </td>
            <td >
              <p
                :class="{
                  exposure: item.Exposure === '노출',
                  hiding: item.Exposure === '숨김',
                }"
              >
                {{ item.Exposure }}
              </p>
            ...
            <td >
               <p
                :class="{
                  Store_entry: item.Store_status === '입점',
                  Not_in_store: item.Store_status === '미입점',
                }"
              >
              {{ item.Store_status }}
              </p></td>
            <td>{{ item.Date }}</td>
            <td>
              <router-link :to="`/modify(partner)/${item.Company_code}`">
              <v-btn
                size="x-small"
                flat
                class="management"
...
const loadItems = async ()=>{
  try{
    ...
    console.log(resp.data.partner);
    items.value = resp.data.partner;
  }
  catch(error){
